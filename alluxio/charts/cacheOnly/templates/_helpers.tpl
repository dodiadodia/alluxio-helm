...
*/}}
{{- define "cacheonly.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "cacheonly.worker.resources" -}}
resources:
  limits:
...
{{- define "cacheonly.master.resources" -}}
resources:
  limits:
...
{{- define "cacheonly.journal.format.resources" -}}
resources:
  limits:
...
{{- define "cacheonly.licenseVolumeMount" -}}
  {{- if .Values.license }}
            - name: secret-cacheonly-license-volume
              mountPath: /secrets/cacheonly-license
              readOnly: true
  {{- end }}
{{- end }}

{{- define "cacheonly.master.secretVolumeMounts" -}}
...
{{- define "cacheonly.worker.secretVolumeMounts" -}}
...
{{- define "cacheonly.master.configmapVolumeMounts" -}}
...
{{- define "cacheonly.worker.configmapVolumeMounts" -}}
...
{{- define "cacheonly.master.otherVolumeMounts" -}}
  {{- range .Values.mounts }}
            - name: "{{ .name }}"
              mountPath: "{{ .path }}"
  {{- end }}
...
{{- define "cacheonly.worker.otherVolumeMounts" -}}
  {{- range .Values.mounts }}
            - name: "{{ .name }}"
              mountPath: "{{ .path }}"
  {{- end }}
...
{{- define "cacheonly.worker.tieredstoreVolumeMounts" -}}
...
{{- define "cacheonly.worker.daemonset.tieredstoreVolumes" -}}
  {{- if .Values.tieredstore.levels }}
    {{- range .Values.tieredstore.levels }}
      {{- if .mediumtype }}
        {{- /* The mediumtype can have multiple parts like MEM,SSD */}}
...
{{- define "cacheonly.worker.statefulset.volumeClaimTemplates" -}}
  {{- if .Values.tieredstore.levels }}
    {{- range .Values.tieredstore.levels }}
      {{- if .mediumtype }}
        {{- /* The mediumtype can have multiple parts like MEM,SSD */}}
      ...
      spec:
        ...
        resources:
          requests:
            ...
            {{- end }}
          {{- end }}
        {{- /* The mediumtype is a single value like SSD. */}}
        {{- else }}
          {{- $mediumName := .mediumtype | lower }}
          {{- if eq .type "persistentVolumeClaim" }}
    - metadata:
...
{{- define "cacheonly.worker.statefulset.tieredstoreVolumes" -}}
  {{- if .Values.tieredstore.levels }}
    {{- range .Values.tieredstore.levels }}
      {{- if .mediumtype }}
        {{- if contains "hostPath" .type }}
          {{- fail "When Alluxio workers are deployed as a Statefulset, directly using hostPath for Alluxio worker storage is not supported, because a worker pod may get rescheduled to another node and lose access to the previous data after restarting. To use hostPath as storage, declare a PVC and use hostPath for the matching PV." }}
        {{- end }}
        {{- /* The mediumtype can have multiple parts like MEM,SSD */}}
        {{- if contains "," .mediumtype }}
          {{- $parts := splitList "," .mediumtype }}
...
{{- define "cacheonly.worker.secretVolumes" -}}
  {{- range $key, $val := .Values.secrets.worker }}
        - name: secret-{{ $key }}-volume
          secret:
            secretName: {{ $key }}
...
{{- define "cacheonly.imagePullSecrets" -}}
imagePullSecrets:
...
*/}}
{{- define "cacheonly.extraVolumeMounts" -}}
...
@param .extraServicePorts   An object representing a list of ports.
                            Each item can contain the following fields:
                              ...
                              item.name
*/}}
{{- define "cacheonly.extraServicePorts" -}}
...
*/}}
{{- define "cacheonly.extraContainers" -}}
